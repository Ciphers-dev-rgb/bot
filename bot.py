...
# Masukkan link gambar dengan format uc?export=download&id=ID_FILE_ANDA atau link /view biasa
...
# --- DATABASE SEMENTARA (Akan disinkronisasi dari Sheets) ---
USERS = {}
TRANSAKSI = {}
PRODUK = {}

# --- FUNGSI ENHANCEMENT: Konversi otomatis Link Google Drive ---
def get_direct_gdrive_link(url):
    """Mengubah link /view Google Drive menjadi link direct download agar bot bisa mengirim gambar"""
...
# Terapkan konversi ke variabel global
BANNER_URL_DIRECT = get_direct_gdrive_link(BANNER_URL)
QRIS_URL_DIRECT = get_direct_gdrive_link(QRIS_URL)

# --- FUNGSI SINKRONISASI DATABASE (BACKGROUND) ---
def load_data_from_db():
    global USERS, TRANSAKSI, PRODUK
    try:
        ...
        logger.info("Menggunakan database lokal/sementara sebagai fallback.")

async def send_to_db(action, data):
    """Fungsi untuk mengirim data ke Google Sheets tanpa membuat bot lemot/lag"""
    def _post():
        try:
    ...
    # Jalankan request HTTP di thread terpisah agar tidak memblokir Telegram Bot
...
def render_product_detail(pid):
    ...
    for variant_name, variant_data in product['variants'].items():
        lines.append(f"┊・ {variant_name}: {variant_data['price']} - Stok: {variant_data['stock']}")
    lines.append("╰ - - - - - - - - - - - - - - - - - - - - - ╯")
    return "\n".join(lines)
...
async def show_admin_manage_products_message(message, page=1):
    await send_banner_message(
        message,
        render_admin_product_list(page),
        reply_markup=get_admin_product_list_keyboard(page),
        parse_mode='Markdown'
    )
    ...
    try:
        if action == "add_product":
            ...
            await send_banner_message(
                ...
                reply_markup=get_admin_product_detail_keyboard(pid),
                parse_mode='Markdown'
            )
            return True
        ...
        if action == "edit_basic":
            ...
            await send_banner_message(
                update.message,
                "✅ Info dasar produk berhasil diperbarui.",
                reply_markup=get_admin_product_detail_keyboard(pid),
                parse_mode='Markdown'
            )
            await send_banner_message(
                update.message,
                render_admin_product_detail(pid),
                reply_markup=get_admin_product_detail_keyboard(pid),
                parse_mode='Markdown'
            )
            return True
        ...
        if action == "variant_upsert":
            ...
            await send_banner_message(
                update.message,
                f"✅ Varian *{variant_name}* berhasil disimpan untuk produk *{PRODUK[pid]['nama']}*.",
                reply_markup=get_admin_variant_keyboard(pid),
                parse_mode='Markdown'
            )
            return True
        ...
        await send_banner_message(
            update.message,
            f"❌ Input tidak valid.\n\nAlasan: `{str(e)}`\n\nKirim ulang sesuai format atau kirim `/cancel` untuk batal.",
            parse_mode='Markdown'
        )
        return True
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "go_home":
        ...
        if jenis_trx == "depo":
            ...
            if metode == "saldo":
                ...
                if saldo_user >= nominal:
                    ...
                    # Sync ke Database (Pembelian Sukses via Saldo)
                    await send_to_db("buy_product", {
                        "user_id": user_id, "pid": pid, "variant": variant_name, 
                        "harga": nominal, "waktu": waktu, "nama_produk": product['nama']
                    })
            ...
            TRANSAKSI[trx_id] = {
                # TAMBAHKAN BARIS DI BAWAH INI JUGA
                "trx_id": trx_id, 
                "jenis": "produk",
                "pid": pid,
        ...
        # Mengirim Gambar QRIS menggunakan Link URL atau Path File
        try:
            if QRIS_URL_DIRECT.startswith("http"):
                await context.bot.send_photo(
                    chat_id=query.message.chat_id,
                    photo=QRIS_URL_DIRECT,
                    caption=pesan,
            ...
            await context.bot.send_message(
                chat_id=query.message.chat_id,
                text="⚠️ *Error Sistem:* Gambar QRIS tidak dapat diakses. Pastikan Link Drive Valid atau file ada.",
                parse_mode='Markdown'
            )
            return
        ...
        context.job_queue.run_once(
            pengingat_trx_job,
            300,
            data={'trx_id': trx_id, 'chat_id': query.message.chat_id},
            name=f"reminder_{trx_id}"
        )
        ...
        if trx and trx['status'] == 'pending':
            ...
            if trx['jenis'] == "deposit":
                deskripsi_trx = "Deposit Saldo"
            else:
                deskripsi_trx = f"Produk: {PRODUK[trx['pid']]['nama']} | Varian: {trx['variant']}"
        ...
        if trx and trx['status'] == 'waiting_admin':
            ...
            if trx['jenis'] == 'deposit':
                ...
                # Sync Pembelian Produk ke Database
                await send_to_db("buy_product", {
                    "user_id": uid, "pid": pid, "variant": variant_name, 
                    "harga": trx['jumlah'], "waktu": trx['waktu'], "nama_produk": product['nama']
                })
        ...
        if user_id != ADMIN_ID:
            return await query.answer("Akses Ditolak!", show_alert=True)
        clear_admin_input_state(context)
        await append_with_banner(
            query,
            context,
            render_admin_product_list(1),
            reply_markup=get_admin_product_list_keyboard(1),
            parse_mode='Markdown'
        )
        ...
        await append_with_banner(
            query,
            context,
            render_admin_variant_detail(pid, variant_name),
            reply_markup=get_admin_variant_detail_keyboard(pid, variant_name),
            parse_mode='Markdown'
        )
        ...
        # Sync Hapus Varian ke DB
        await send_to_db("delete_variant", {"pid": pid, "variant": variant_name})

        await append_with_banner(
            query,
            context,
            f"✅ Varian *{variant_name}* berhasil dihapus dari produk *{PRODUK[pid]['nama']}*.",
            reply_markup=get_admin_variant_keyboard(pid),
            parse_mode='Markdown'
        )
        ...
        await append_with_banner(
            query,
            context,
            f"✅ Produk *{nama}* dengan kode `{pid}` berhasil dihapus.",
            reply_markup=get_admin_product_list_keyboard(1),
            parse_mode='Markdown'
        )
...
def main():
    # Ambil data dari Google Sheets sebelum bot dinyalakan (agar memory fresh)
    load_data_from_db()
    ...
    # --- KONFIGURASI WEBHOOK & POLLING ---
    # Port disediakan otomatis oleh platform cloud (seperti Render/Heroku)
    PORT = int(os.environ.get('PORT', '8443'))
    
    # URL Webhook aplikasi Anda (diisi otomatis dari Environment Variables Cloud Hosting)
    WEBHOOK_URL = os.environ.get('WEBHOOK_URL', '').strip()

    if WEBHOOK_URL:
        print(f"Mulai mode WEBHOOK di port {PORT}...")
        # Menjalankan bot dengan Webhook untuk Cloud Hosting
